...
export default class AuthController {
    ...
    public async register({request, auth}: HttpContextContract) {
        ...
        const password = request.input('password')
        const tipo = request.input('perfil')  /* Este campo guarda se ingresa el tipo de perfil, para crearlo a la hora de crear el usuario*/

        if (tipo!= 'Admin'&& tipo!='Dev'&&tipo!='Avg_user'){ /* Son los 3 tipos de perfiles permitidos*/
            return{
        ...
        try{
            const usuarioExistente: number = await this.getValidarUsuarioExistente(identificacion)
                if (usuarioExistente === 0){
                    const perfil = new Perfil() /* Antes de crear el usuario se crea su perfil, que tiene un id automatico y la descripcion es el tipo de perfil */
